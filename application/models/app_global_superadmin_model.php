...
class app_global_superadmin_model extends CI_Model {
	...
	public function generate_index_produk($limit,$offset,$filter)
	{
		$hasil="";
		$query_add = "";
		if(!empty($filter))
		...
		$tot_hal = $this->db->query("select * from 
		dlmbg_produk ".$query_add." order by id_produk DESC");

		$config['base_url'] = base_url() . 'superadmin/produk/index/';
		$config['total_rows'] = $tot_hal->num_rows();
		$config['per_page'] = $limit;
		$config['uri_segment'] = 4;
		$config['first_link'] = 'First';
		...
		$hasil .= "<table class='table table-striped table-condensed'>
					...
					<th>Diskon</th>
					<th>Dihapus</th>
					<th width='110'><a href='".base_url()."superadmin/produk/tambah' class='btn btn'><i class='icon-plus'></i> Tambah</a></th>
					</tr>
					</thead>";
		$i = $offset+1;
		foreach($w->result() as $h)
		{
			...
			$hasil .= "<a href='".base_url()."superadmin/produk/edit/".$h->id_produk."' class='btn btn-small'><i class='icon-edit'></i></a>";
			$hasil .= "<a href='".base_url()."superadmin/produk/hapus/".$h->id_produk."' onClick=\"return confirm('Anda yakin?');\" class='btn btn-small'><i class='icon-trash'></i></a></td>
					</tr>";
			$i++;
		}
		$hasil .= '</table>';
