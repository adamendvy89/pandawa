...
class produk_dinas extends MX_Controller {
   ...
   {  
      ...
      $d['dt_index_produk'] = $this->app_global_model->generate_index_produk_dinas($id_param,$this->config->item("limit_item_medium"),$uri);
	  ...
	  $this->breadcrumb->append_crumb("produk Dinas", '/');
      ...
      $this->load->view($_SESSION['site_theme'].'/bidang/produk_dinas/bg_home');
      $this->load->view($_SESSION['site_theme'].'/bg_right');
      $this->load->view($_SESSION['site_theme'].'/bg_footer');
   }
 
   public function detail($id_produk,$tipe)
   {
      $where['id_multi_produk'] = $id_produk;
      $get_data = $this->db->get_where("dlmbg_multi_produk",$where);
      $ret_data = $get_data->row();
      if($get_data->num_rows()>0)
      {
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Indexs produk', base_url().'web/produk');
			...
			$d['dt_detail_produk'] = $this->app_global_model->generate_detail_produk($id_produk,$tipe);

			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/bg_left');
      $this->load->view($_SESSION['site_theme'].'/bidang/produk_dinas/bg_detail');
			$this->load->view($_SESSION['site_theme'].'/bg_right');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
