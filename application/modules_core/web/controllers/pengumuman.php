...
class pengumuman extends MX_Controller {
   ...
   public function index($uri=0)
   {
      ...
      $this->breadcrumb->append_crumb('Indexs Pengumuman', '/');
      ...
      $d['dt_index_pengumuman'] = $this->app_global_model->generate_index_pengumuman($this->config->item('limit_item_medium'),$uri,$filter);
      ...
      $this->load->view($_SESSION['site_theme'].'/pengumuman/bg_home');
   ...
   public function detail($id_pengumuman,$tipe)
   {
      ...
      if($get_data->num_rows()>0)
      {
			...
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/bg_left');
			$this->load->view($_SESSION['site_theme'].'/pengumuman/bg_detail');
			$this->load->view($_SESSION['site_theme'].'/bg_right');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
