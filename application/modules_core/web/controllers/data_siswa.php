...
class data_siswa extends MX_Controller {
   ...
   public function index($uri=0)
   {
		...
		$d['dt_data_siswa'] = $this->app_global_model->generate_index_siswa($this->config->item("limit_item_medium"),$uri,$filter);

		$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
		if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
		$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
		$this->load->view($_SESSION['site_theme'].'/data_sekolah/data_siswa/bg_home');
   ...
   public function sekolah($id_param,$uri=0)
   {
      ...
      if($get_data->num_rows()>0)
      {
			...
			$d['id_sekolah'] = $ret_data->id_sekolah_profil;
      		$d['dt_data_siswa'] = $this->app_global_model->generate_index_siswa_sekolah($id_param,$this->config->item("limit_item_medium"),$uri);

			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/data_sekolah/data_siswa/bg_home_sekolah');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
      {
